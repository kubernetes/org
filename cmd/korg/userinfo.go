...
const (
	...
	// unknownLookupError is reported when a lookup yields neither a result nor an
	// error. findUserDetails never returns that combination today; both output modes
	// carry the fallback so that if the invariant ever breaks, the user is still
	// reported instead of silently disappearing.
	unknownLookupError = "unknown error"
)
...
// match a query for "- alice"). Trailing inline comments and surrounding quotes are
// stripped, so "- alice", `- "alice"` and "- alice # area expert" all match.
func hasRealMatch(matches []*houndindex.Match, username string) bool {
	for _, m := range matches {
		...
		// Drop any trailing inline comment before comparing; usernames cannot contain "#".
...
func runUserinfo(ctx context.Context, repoRoot string, usernames []string, outputJSON, verifyOwners bool, w io.Writer) error {
	...
	// Use a dedicated http.Client for GitHub and a separate one for hound.
	// newGitHubClient layers a token-injecting RoundTripper onto the client it is
	// handed, so keeping hound on its own client guarantees the GitHub token is
	// never sent to cs.k8s.io, independent of go-github's internal copy semantics.
	ghHTTP := &http.Client{Timeout: defaultHTTPTimeout}
	houndHTTP := &http.Client{Timeout: defaultHTTPTimeout}

	gh, err := newGitHubClient(ghHTTP)
	if err != nil {
	...
	for i, name := range usernames {
		g.Go(func() error {
			info, err := findUserDetails(gctx, gh, houndHTTP, configs, name, verifyOwners)
			if err != nil {
				errs[i] = err
				return nil
			}
	...
	if outputJSON {
		...
		for i, r := range results {
			...
			default:
				out[i] = &UserDetails{Username: usernames[i], Error: unknownLookupError}
			}
		}
		...
		// Mirrors the JSON branch above, including the defensive fallback, so a
		// (nil, nil) result is never silently omitted from either output mode.
		for i, r := range results {
			switch {
			case r != nil:
				r.renderText(w)
			case errs[i] != nil:
				fmt.Fprintf(w, "\n=== %s\nERROR: %v\n", usernames[i], errs[i])
			default:
				fmt.Fprintf(w, "\n=== %s\nERROR: %s\n", usernames[i], unknownLookupError)
			}
		}
	}
